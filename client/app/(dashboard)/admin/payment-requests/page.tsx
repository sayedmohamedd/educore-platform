...
import { mockPaymentRequests } from "./_components/mock-data";
import { PaymentRequest, PaymentRequestFilter } from "./_components/types";

import PaymentRequestCard from "./_components/PaymentRequestCard";
import PaymentRequestDialog from "./_components/PaymentRequestDialog";
import PaymentRequestFilters from "./_components/PaymentRequestFilters";
...
const AdminPaymentRequestsPage = () => {
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await adminClientService.getPaymentRequests("ALL");
        console.log("Fetched payment requests:", data.payments);
  ...
  const filteredRequests = useMemo(() => {
    ...
    return requests.filter((request) => {
      ...
      const matchesSearch =
        !normalizedSearch ||
        request.student.name.toLowerCase().includes(normalizedSearch) ||
        request.student.email.toLowerCase().includes(normalizedSearch) ||
        request.course.title.toLowerCase().includes(normalizedSearch) ||
        request.transferReference.toLowerCase().includes(normalizedSearch);
